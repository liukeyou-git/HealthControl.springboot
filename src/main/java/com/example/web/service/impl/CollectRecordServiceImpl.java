 ...
 */
@Service
public class CollectRecordServiceImpl extends ServiceImpl<CollectRecordMapper, CollectRecord>
        implements CollectRecordService {
    ...
    private LambdaQueryWrapper<CollectRecord> BuilderQuery(CollectRecordPagedInput input) {
        ...
        if (input.getRelativeId() != null) {
            queryWrapper = queryWrapper.eq(CollectRecord::getRelativeId, input.getRelativeId());
        }
     ...
     * 处理收藏记录对于的外键数据
     */
    private List<CollectRecordDto> DispatchItem(List<CollectRecordDto> items)
        ...
        for (CollectRecordDto item : items) {
            ...
            if ("健康知识".equals(item.getCollectType())) {
            ...
            } else if ("食谱".equals(item.getCollectType())) {
